...
def run_cycle(
    ...
    # ── STEP 1: Fetch live MT5 data ──────────────────────────────────────────
    with MT5Lock():
        try:
            # Ensure we are logged into the correct account before getting data
    ...
    while not _shutdown_requested:
        ...
        for account_id in enabled_accounts:
            ...
            if not state["initialized"]:
                ...
                with MT5Lock():
                    try:
                        checks_passed = startup_checks(connector, acct_settings)
                    finally:
                        connector.disconnect()
            ...
            for symbol in trading_symbols:
                if _shutdown_requested:
                    break
                try:
                    run_cycle(symbol, connector, risk_mgr, trade_logger, trade_memory, active_manager, acct_settings, global_ai_cache, login_val, password_val, server_val, path_val)
                except Exception as e:
